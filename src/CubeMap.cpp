...
void CubeMap::render( float* inverseTransMatrix, float& aCameraAngle )
{
	...
	//	glLoadIdentity();
		...
		{
			glLoadIdentity();
			glRotatef( aCameraAngle, 0.0f, 1.0f, 0.0f );
...
void CubeMap::setupCompressedCubeMap( string filenamePrefix )
{
	...
	unsigned char* faces[6];// = (unsigned char**)malloc(sizeof(int) * 6);
...
void CubeMap::setupCubeMap(string filenamePrefix)
{
	...
	float* faces[6];// = (float**)malloc(sizeof(int) * 6);
