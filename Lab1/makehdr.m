function hdr = makehdr(infile, min_exp, curve, outfile);
...
while( filename ~= -1 )
    ...
    for i = 1:height
        for j = 1:width
            ...
            % for each wavelength
            for c = 1:3
                numerator(i, j, c) = numerator(i, j, c) + g(c) * (Xij(c) / exposureTime);
                denominator(i, j, c) = denominator(i, j, c) + g(c);
            end
        end
    end  
