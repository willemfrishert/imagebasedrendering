 ...
 * @param negativeZ
 */
void HDRLoader::extractFaces(const HDRLoaderResult &res, float **positiveX, float **negativeX, 
						float **positiveY, float **negativeY, float **positiveZ, float **negativeZ)
{
	int side = 256;
	int ch = 3;

	*positiveX = new float[side * side * ch];
	*positiveY = new float[side * side * ch];
	*positiveZ = new float[side * side * ch];
	*negativeX = new float[side * side * ch];
	*negativeY = new float[side * side * ch];
	*negativeZ = new float[side * side * ch];

	int start = side * ch;
	for (int i = 0; i < side; i++)
	{
		int index	= i * res.width * ch;
